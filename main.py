...
try:
    ...
    CHANNEL_NAME = config("CHANNEL_NAME", default="@jr_bots")
...
async def check_bots():
    ...
    new_message = (
        "**🛟 New periodic check in progress...**\n\n" + channel_current_msg.text
    )
    try:
    ...
    # form the message
    status_message = f"**{CHANNEL_NAME}** **- Bot Status**\n\n"
    for bot, value in bot_stats.items():
        status_message += (
            f"<b>» {value['status']} @{bot}</b>\n"
            if bot_stats[bot]["response_time"] is None
            else f"<b>» {bot_stats[bot]['status']} @{bot} [ {bot_stats[bot]['response_time']} ]</b>\n"
        )
    ...
    status_message += f"\n\n**🔁 The Bot status will be updated in every 6 hours.**"
